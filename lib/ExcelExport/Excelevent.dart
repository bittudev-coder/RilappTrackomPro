...
class ExcelExporterEvent {
  Future<void> excelEvent(List<Event> _eventList,String VehiclesNo) async {
    ...
    sheet.getRangeByName('B1').setText("Event");
    sheet.getRangeByName('B2').setText(VehiclesNo);
    sheet.getRangeByName('B3').setText(formatTime(_eventList.first.eventTime!));
    sheet.getRangeByName('B4').setText(formatTime(_eventList.last.eventTime!));
    ...
    sheet.getRangeByName('B6').setText("Event");
    ...
    for (int i = 0; i < _eventList.length; i++) {
      sheet.getRangeByIndex(i + 7, 1).setText(formatTime(_eventList[i].eventTime!));
      sheet.getRangeByIndex(i + 7, 2).setValue((_eventList[i].type)!.tr);
    ...
    try {
      ...
      final String sanitizedDate = formatDate(_eventList.first.eventTime!).replaceAll(RegExp(r'[^\w\s]'), '_'); // Sanitize date
