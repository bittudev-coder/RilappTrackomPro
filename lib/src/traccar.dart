...
class Traccar {
  ...
  static Future<http.Response?> login(email, password) async {
    if (UserRepository.getServerUrl() != null) {
      serverURL = UserRepository.getServerUrl().toString();
  ...
  static Future<Event?> getEventById(String id) async {
    ...
    /// [body] sets the body of the request. It can be a [String], a [List<int>] or
    /// a [Map<String, String>]. If it's a String, it's encoded using [encoding] and
    /// used as the body of the request. The content-type of the request will   /// default to "text/plain".
    ///
    /// If [body] is a List, it's used as a list of bytes for the body of the
    /// request.
    ///
  ...
  static Future<VehicleData?> getDeviceData(String id) async {
  final response = await http.get(Uri.parse(serverURL! + "/api/devices/"+id),
  headers: headers);
  if (response.statusCode == 200) {// Print response to check format
  Map<String, dynamic> jsonResponse = json.decode(response.body);
  return VehicleData.fromJson(jsonResponse);
  } else {
  ...
  static Future<String?> getDeviceDataJson(String id) async {
    final response = await http.get(
      Uri.parse('$serverURL/api/devices/$id'),
      headers: headers,
    );
