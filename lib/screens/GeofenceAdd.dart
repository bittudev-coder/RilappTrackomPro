...
class _GeofenceAddPageState extends State<GeofenceAddPage> {
  ...
  Widget buildMap(DataController controller) {
    device = controller.devices[args!.deviceId] as Device;
    if (controller.positions.containsKey(args!.deviceId)) {
      if (!pageDestoryed) {
        if (_markers.isEmpty) {
          updateMarker(controller.positions[args!.deviceId] as PositionModel);
        }
      }
      return Scaffold(
